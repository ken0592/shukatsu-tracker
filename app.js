...
const quoteMessages = quoteSubjects.flatMap((subject) => quoteEndings.map((ending) => `${subject}${ending}`));
...
function renderDailyQuote() {
  ...
  els.dailyQuote.innerHTML = `
    ...
    <figure>
      ...
      <figcaption><span>${formatQuoteDate(today)}</span> ・ <cite>${escapeHtml(quote.author)}</cite></figcaption>
    </figure>
  `;
}
...
function buildDailyQuotes() {
  ...
  quoteMonthDays.forEach((daysInMonth, monthIndex) => {
    ...
    for (let day = 1; day <= daysInMonth; day += 1) {
      ...
      const message = quoteMessages[sequence];
      const author = quoteAuthors[sequence % quoteAuthors.length];
      quotes[key] = {
        message,
        author: `${author}風`
      };
