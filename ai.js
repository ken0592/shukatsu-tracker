(function setupAiClient(global) {
  ...
  const faqItems = [
    ...
    },
    {
      ...
      answer: "「ESの型」の「＋ 型」からよく使う文章を保存できます。一覧はタイトルだけを表示し、押すと本文・編集・削除を開けます。企業詳細で型を選んで「この回答に入れる」を押すと、その回答だけに追加できます。スマホでは画面上部の「ESの型」で切り替えます。"
    },
    {
      topic: "締切と予定",
      keywords: ["締切", "予定", "カレンダー", "面接日", "近日", "今日やる"],
      answer: "締切日を入れると、小型カレンダーに赤い点が付きます。登録済みの面接などの予定は青い点です。日付を押すとその日の予定をすべて開け、企業を押すと詳細へ移れます。前月・翌月ボタンで別の月も確認できます。スマホでは画面上部の「カレンダー」で切り替えます。"
    },
    {
      topic: "ログインと同期",
    ...
    },
    {
      ...
      answer: "進捗は「気になる・応募済み・選考中・採用・不採用」の5つです。企業カードの進捗を押して選ぶだけで自動保存されます。削除したカードはゴミ箱から戻せます。"
    }
  ];
