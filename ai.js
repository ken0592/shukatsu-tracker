(function setupAiClient(global) {
  ...
  const faqItems = [
    ...
    },
    {
      ...
      answer: "画面上部の「＋追加」から企業カードを作れます。企業名だけでも保存でき、締切・選考区分・志望度はあとから編集できます。志望度は最優先・高・中・低の4段階です。企業詳細ではESとその他メモを編集できます。企業内の＋から選考を追加できます。"
    },
    {
      topic: "ESの質問と回答",
    ...
    },
    {
      ...
      answer: "「ES・ガクチカの型」の＋を押すと入力欄が開き、よく使う文章を保存できます。企業詳細でES質問カードを開き、型を選んで「この回答に入れる」を押すと、その回答だけに追加できます。"
    },
    {
      topic: "締切と予定",
      keywords: ["締切", "予定", "カレンダー", "面接日", "近日", "今日やる"],
      answer: "締切日を入れると、カレンダーに表示されます。登録済みの面接などの予定もカレンダーに残ります。前月・翌月ボタンで別の月も確認できます。"
    },
    {
      topic: "ログインと同期",
    ...
    },
    {
      ...
      answer: "進捗は「気になる・応募済み・選考中・採用・不採用」の5つです。企業カードの進捗を押して選ぶだけで自動保存されます。選考を開いて「冬インターンを削除」など対象名付きのボタンを押すと、その選考だけをゴミ箱に移します。他の選考のES・メモは残ります。削除した選考はゴミ箱や企業の＋から復元できます。未保存の詳細がある場合は先に保存します。"
    }
  ];
      ...
      cards: {
        ...
        items: {
          ...
          properties: {
            ...
            status: { type: "string", enum: ["気になる", "応募済み", "選考中", "採用", "不採用"] },
      ...
      messages: [
        {
          ...
          content: [
            ...
            "statusは気になる・応募済み・選考中・採用・不採用の5種類です。メモに状態が明記されている場合だけ設定します。応募やES提出済みは応募済み、面接や結果待ちは選考中、内定やインターン参加決定は採用、落選や辞退は不採用。ES締切だけの記述や状態が不明な場合は必ず気になるにしてください。",
            "priorityは志望度が明記されている場合だけ最優先・高・中・低にします。第一志望は最優先、不明なら必ず未定にしてください。",
